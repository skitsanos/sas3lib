...
package com.skitsanos.drawing
{
	...
	public class DashedLine extends Sprite
	{
		...
		private var remainingDist:int = 0;	// stores distance between the end of the last full dash or gap and the end coordinates specified in lineTo
		private var curIndex:int = -1;	// current index in the length array, so we know which dash or gap to draw
		private var arraySum:int = 0;	// total length of the dashes and gaps... not currently being used for anything, but maybe useful?
		...
		public function DashedLine(weight:int = 0, color:int = 0, lengthsArray:Array = null)
		{
		...
		public function lineTo(x:int, y:int):void
		{
			...
			outerLoop : while (Math.abs(startX - curX) < Math.abs(startX - x) || Math.abs(startY - curY) < Math.abs(startY - y))
			{
				...
				{
					var dist:int = (remainingDist == 0) ? lengthsArray[i] : remainingDist;	// distance to draw is either the dash/gap length from the array or remainingDist left over from the last lineTo if there is any
					// get increments of x and y based on distance, slope, and direction - see getCoords()
					var xInc:int = getCoords(dist, slope).x * xDir;
					var yInc:int = getCoords(dist, slope).y * yDir;
		...
		public function lineStyle(w:int = 0, c:int = 0, a:int = 1):void
		{
			lineWeight = w;
			lineColor = c;
			lineAlpha = a;
		...
		// basic beginFill
		public function beginFill(c:int, a:int = 1):void
		{
